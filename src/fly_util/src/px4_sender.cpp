...
fly_util::command Command_Now; // 无人机当前执行命令

// Command Last [from upper node]
fly_util::command Command_Last; // 无人机上一条执行命令

float get_ros_time(ros::Time begin);
void prinft_command_state();

void Command_cb(const fly_util::command::ConstPtr &msg)
{
    Command_Now = *msg;
}
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主 函 数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
int main(int argc, char **argv)
{
    ...
    ros::Subscriber Command_sub = nh.subscribe<fly_util::command>("/px4/command", 10, Command_cb);
...
void prinft_command_state()
{
    ...
    if ((sub_mode & 0b10) == 0) // xy channel
    {
        ...
        cout << "X_setpoint   : " << Command_Now.pos_sp[0] << " [ m ]"
             << "  Y_setpoint : " << Command_Now.pos_sp[1] << " [ m ]" << endl;
    }
    else
    {
        cout << "Submode: xy velocity control " << endl;
        cout << "X_setpoint   : " << Command_Now.vel_sp[0] << " [m/s]"
             << "  Y_setpoint : " << Command_Now.vel_sp[1] << " [m/s]" << endl;
    }
